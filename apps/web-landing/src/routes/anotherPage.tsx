  ...
  return (
    <main className="landing-shell">
      <section className="status-card">
        ...
        <React.Suspense fallback={<p>Loading Convex data...</p>}>
          <ConvexDemoPanel />
        </React.Suspense>
        <Link className="button button-ghost" to="/">
          Back to Landing
...
function ConvexDemoPanel() {
  ...
  const [mutationState, setMutationState] = React.useState<"idle" | "saving" | "saved" | "error">(
    "idle",
  );

  const onAddNumber = React.useCallback(async () => {
