...
function installMockBridge() {
  ...
  browserWindow.ggResolveCapturePreviewURL = async () => livePreviewImageURL;
...
test.beforeEach(async ({ page }) => {
  await page.route(`${livePreviewRouteOrigin}/**`, async (route) => {
    await route.fulfill({
      status: 200,
      ...
      body: Buffer.from(livePreviewImageBase64, "base64"),
    });
  });
