...
where
    ...
    A: apis::agent::Agent<E, Claims = C> + apis::capture::Capture<E, Claims = C> + apis::export::Export<E, Claims = C> + apis::permissions::Permissions<E, Claims = C> + apis::project::Project<E, Claims = C> + apis::recording::Recording<E, Claims = C> + apis::sources::Sources<E, Claims = C> + apis::system::System<E, Claims = C> + apis::ApiAuthBasic<Claims = C> + Send + Sync + 'static,
    E: std::fmt::Debug + Send + Sync + 'static,
    C: Send + Sync + 'static,
{
    // build our application with a route
    Router::new()
        .route("/v1/agent/preflight",
            post(agent_agent_preflight::<I, A, E, C>)
        )
        .route("/v1/agent/runs",
            post(agent_agent_run::<I, A, E, C>)
        )
        .route("/v1/agent/runs/{job_id}",
            get(agent_agent_status::<I, A, E, C>)
        )
        .route("/v1/agent/runs/{job_id}/apply",
            post(agent_agent_apply::<I, A, E, C>)
        )
        .route("/v1/capture/preview-frame",
            get(capture_capture_preview_frame::<I, A, E, C>)
        )
        .route("/v1/capture/start-current-window",
            post(capture_capture_start_current_window::<I, A, E, C>)
        )
        .route("/v1/capture/start-display",
            post(capture_capture_start_display::<I, A, E, C>)
        )
        .route("/v1/capture/start-window",
            post(capture_capture_start_window::<I, A, E, C>)
        )
        .route("/v1/capture/status",
            get(capture_capture_status::<I, A, E, C>)
        )
        .route("/v1/capture/stop",
            post(capture_capture_stop::<I, A, E, C>)
        )
        .route("/v1/engine/capabilities",
            get(system_engine_capabilities::<I, A, E, C>)
        )
        .route("/v1/export/info",
            get(export_export_info::<I, A, E, C>)
        )
        .route("/v1/exports",
            post(export_export_run::<I, A, E, C>)
        )
        .route("/v1/exports/from-cut-plan",
            post(export_export_run_cut_plan::<I, A, E, C>)
        )
        .route("/v1/exports/{job_id}",
            get(export_export_get::<I, A, E, C>)
        )
        .route("/v1/permissions",
            get(permissions_permissions_get::<I, A, E, C>)
        )
        .route("/v1/permissions/input-monitoring/open-settings",
            post(permissions_permissions_open_input_monitoring_settings::<I, A, E, C>)
        )
        .route("/v1/permissions/input-monitoring/request",
            post(permissions_permissions_request_input_monitoring::<I, A, E, C>)
        )
        .route("/v1/permissions/microphone/request",
            post(permissions_permissions_request_microphone::<I, A, E, C>)
        )
        .route("/v1/permissions/screen-recording/request",
            post(permissions_permissions_request_screen_recording::<I, A, E, C>)
        )
        .route("/v1/project/current",
            get(project_project_current::<I, A, E, C>)
        )
        .route("/v1/project/open",
            post(project_project_open::<I, A, E, C>)
        )
        .route("/v1/project/recents",
            get(project_project_recents::<I, A, E, C>)
        )
        .route("/v1/project/save",
            post(project_project_save::<I, A, E, C>)
        )
        .route("/v1/recording/start",
            post(recording_recording_start::<I, A, E, C>)
        )
        .route("/v1/recording/stop",
            post(recording_recording_stop::<I, A, E, C>)
        )
        .route("/v1/sources",
            get(sources_sources_list::<I, A, E, C>)
        )
        .route("/v1/system/ping",
            get(system_system_ping::<I, A, E, C>)
        )
        .with_state(api_impl)
}
...
fn agent_agent_apply_validation(
  ...
  models::AgentAgentApplyPathParams,
        models::AgentApplyPayload,
), ValidationErrors>
{
...
let result = api_impl.as_ref().agent_agent_apply(
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    agent_agent_preflight_validation(
          body,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().agent_agent_preflight(
...
let result = api_impl.as_ref().agent_agent_run(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                ...
                                                apis::agent::AgentAgentRunResponse::Status409_EngineConflictErrorResponseBody
                                                    (body)
                                                => {
                                                  let mut response = response.status(409);
                                                  {
...
fn agent_agent_status_validation(
  ...
  models::AgentAgentStatusPathParams,
), ValidationErrors>
{
  path_params.validate()?;
...
let result = api_impl.as_ref().agent_agent_status(
...
#[tracing::instrument(skip_all)]
fn capture_capture_preview_frame_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    capture_capture_preview_frame_validation(
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().capture_capture_preview_frame(
...
fn capture_capture_start_current_window_validation(
        ...
        models::CaptureStartCurrentWindowPayload,
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    capture_capture_start_current_window_validation(
          body,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().capture_capture_start_current_window(
...
fn capture_capture_start_display_validation(
        ...
        models::CaptureStartDisplayPayload,
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    capture_capture_start_display_validation(
          body,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().capture_capture_start_display(
...
fn capture_capture_start_window_validation(
        ...
        models::CaptureStartWindowPayload,
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    capture_capture_start_window_validation(
          body,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().capture_capture_start_window(
...
#[tracing::instrument(skip_all)]
fn capture_capture_status_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    capture_capture_status_validation(
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().capture_capture_status(
...
#[tracing::instrument(skip_all)]
fn capture_capture_stop_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    capture_capture_stop_validation(
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().capture_capture_stop(
...
fn export_export_get_validation(
  ...
  models::ExportExportGetPathParams,
), ValidationErrors>
{
  path_params.validate()?;
...
let result = api_impl.as_ref().export_export_get(
...
#[tracing::instrument(skip_all)]
fn export_export_info_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
let result = api_impl.as_ref().export_export_info(
...
let result = api_impl.as_ref().export_export_run(
...
fn export_export_run_cut_plan_validation(
        ...
        models::ExportRunCutPlanPayload,
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    export_export_run_cut_plan_validation(
          body,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().export_export_run_cut_plan(
...
#[tracing::instrument(skip_all)]
fn permissions_permissions_get_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    permissions_permissions_get_validation(
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().permissions_permissions_get(
...
let result = api_impl.as_ref().permissions_permissions_open_input_monitoring_settings(
...
let result = api_impl.as_ref().permissions_permissions_request_input_monitoring(
...
let result = api_impl.as_ref().permissions_permissions_request_microphone(
...
let result = api_impl.as_ref().permissions_permissions_request_screen_recording(
...
#[tracing::instrument(skip_all)]
fn project_project_current_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    project_project_current_validation(
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().project_project_current(
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    project_project_open_validation(
          body,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().project_project_open(
...
fn project_project_recents_validation(
  ...
  models::ProjectProjectRecentsQueryParams,
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    project_project_recents_validation(
        query_params,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().project_project_recents(
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    project_project_save_validation(
          body,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().project_project_save(
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    recording_recording_start_validation(
          body,
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().recording_recording_start(
...
#[tracing::instrument(skip_all)]
fn recording_recording_stop_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    recording_recording_stop_validation(
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().recording_recording_stop(
...
#[tracing::instrument(skip_all)]
fn sources_sources_list_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    sources_sources_list_validation(
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().sources_sources_list(
...
#[tracing::instrument(skip_all)]
fn system_engine_capabilities_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    system_engine_capabilities_validation(
    )
  ).await.unwrap();
...
let result = api_impl.as_ref().system_engine_capabilities(
...
#[tracing::instrument(skip_all)]
fn system_system_ping_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
let result = api_impl.as_ref().system_system_ping(
